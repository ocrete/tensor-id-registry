...
Neighbor-assisted local X offsets for PIPNet. These channels allow each landmark to
predict positions for its `NUM_NB` nearest landmarks, then merge those predictions
with each landmark's own estimate.

## Neighbor Local X Offset Tensor
...
`nb_x_lk = (col_l + nb_x[c, row_l, col_l]) / FEAT_W`
...
|---|---|---|---|
|0|NBX(0,0,0)|channel-0 cell-(0,0) neighbor-x-offset|tensor-start, channel-0 plane-start|
|...|...|...|...|
|SPATIAL_SIZE x 1 - 1|NBX(0,FEAT_H-1,FEAT_W-1)|channel-0 last-cell neighbor-x-offset|channel-0 plane-end|
|SPATIAL_SIZE x 1|NBX(1,0,0)|channel-1 cell-(0,0) neighbor-x-offset|channel-1 plane-start|
|...|...|...|...|
|SPATIAL_SIZE x CHANNELS - 1|NBX(CHANNELS-1,FEAT_H-1,FEAT_W-1)|last channel last-cell neighbor-x-offset|tensor-end|
