...
Per-landmark local X offsets predicted at each feature-map cell. During decoding,
the selected cell index from [cls_map] is refined with this offset.

## Local X Offset Tensor
...
X estimate before neighbor-averaging is:

`x_l = (col_l + offset_x[l, row_l, col_l]) / FEAT_W`
...
|---|---|---|---|
|0|DX(0,0,0)|landmark-0 cell-(0,0) x-offset|tensor-start, landmark-0 plane-start|
|...|...|...|...|
|SPATIAL_SIZE x 1 - 1|DX(0,FEAT_H-1,FEAT_W-1)|landmark-0 last-cell x-offset|landmark-0 plane-end|
|SPATIAL_SIZE x 1|DX(1,0,0)|landmark-1 cell-(0,0) x-offset|landmark-1 plane-start|
|...|...|...|...|
|SPATIAL_SIZE x NUM_LMS - 1|DX(NUM_LMS-1,FEAT_H-1,FEAT_W-1)|last landmark last-cell x-offset|tensor-end|
